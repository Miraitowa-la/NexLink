...
#define TUD_CMSIS_DAP_V2_BULK_DESCRIPTOR(_itfnum, _stridx, _epout, _epin, _epsize) \
    9, TUSB_DESC_INTERFACE, _itfnum, 0, 2, TUSB_CLASS_VENDOR_SPECIFIC,              \
        CMSIS_DAP_V2_BULK_IFACE_SUBCLASS, CMSIS_DAP_V2_BULK_IFACE_PROTOCOL, _stridx, \
...
static uint8_t const s_configuration_descriptor[] = {
    ...
    TUD_CMSIS_DAP_V2_BULK_DESCRIPTOR(ITF_NUM_CMSIS_DAP_V2_BULK,
                                     USB_STRID_CMSIS_DAP_V2_BULK,
                                     EPNUM_CMSIS_DAP_V2_BULK,
                                     0x80 | EPNUM_CMSIS_DAP_V2_BULK,
                                     CMSIS_DAP_V2_BULK_EPSIZE),
    TUD_HID_INOUT_DESCRIPTOR(ITF_NUM_CMSIS_DAP_V1_HID,
                             USB_STRID_CMSIS_DAP_V1_HID,
                             HID_ITF_PROTOCOL_NONE,
                             sizeof(s_hid_report_descriptor),
                             EPNUM_CMSIS_DAP_V1_HID,
                             0x80 | EPNUM_CMSIS_DAP_V1_HID,
                             CFG_TUD_HID_EP_BUFSIZE,
                             1),
    TUD_CDC_DESCRIPTOR(ITF_NUM_CDC_CONTROL,
                       USB_STRID_CDC,
                       0x80 | EPNUM_CDC_NOTIFICATION,
                       8,
                       EPNUM_CDC_DATA,
                       0x80 | EPNUM_CDC_DATA,
...
static bool cdc_map_line_coding(const cdc_line_coding_t *line_coding,
                                ...
                                uart_stop_bits_t *stop_bits)
{
    ...
    case CDC_LINE_CODING_PARITY_NONE: *parity = UART_PARITY_DISABLE; break;
    ...
    case CDC_LINE_CODING_STOP_BITS_1: *stop_bits = UART_STOP_BITS_1; break;
    case CDC_LINE_CODING_STOP_BITS_1_5: *stop_bits = UART_STOP_BITS_1_5; break;
    case CDC_LINE_CODING_STOP_BITS_2: *stop_bits = UART_STOP_BITS_2; break;
    default: return false;
    }
...
void tud_cdc_line_coding_cb(uint8_t itf, cdc_line_coding_t const *line_coding)
{
    ...
    if (s_cdc_target && cdc_map_line_coding(line_coding, &data_bits, &parity, &stop_bits)) {
        ...
        s_frontend.line_coding(line_coding->bit_rate, line_coding->data_bits,
                               line_coding->parity, line_coding->stop_bits, s_frontend.context);
    }
}
...
void tud_cdc_rx_cb(uint8_t itf)
{
    ...
    while (tud_cdc_n_available(CDC_INSTANCE)) {
        ...
        if (length > 0) {
            if (s_cdc_target) bridge_target_write(s_cdc_target, buffer, length);
            else if (s_frontend.cdc_rx) s_frontend.cdc_rx(buffer, length, s_frontend.context);
        }
    }
}
...
static void cdc_sender_task(void *parameter)
{
    ...
    for (;;) {
        size_t length = 0;
        uint8_t *data = xRingbufferReceiveUpTo(s_cdc_uart_to_usb, &length, portMAX_DELAY,
                                                sizeof(tx_buffer));
        if (!data) {
            continue;
        }
        ...
        for (size_t sent = 0; sent < length;) {
            ...
            if (written == 0 ||
                xSemaphoreTake(s_cdc_tx_done, pdMS_TO_TICKS(CDC_TX_TIMEOUT_MS)) != pdTRUE) {
...
esp_err_t usb_device_init_serial_number(void)
{
    ...
    snprintf(s_serial_descriptor, sizeof(s_serial_descriptor),
...
esp_err_t usb_device_start(const usb_device_config_t *config, bridge_target_t *cdc_target)
{
    ...
    if (xTaskCreatePinnedToCore(usb_device_task, "usb_device_task", 4 * 1024, NULL, 5, NULL,
                                NEXLINK_CPU_WIRELESS) != pdPASS) {
        return ESP_ERR_NO_MEM;
    }
    if (xTaskCreatePinnedToCore(cdc_sender_task, "cdc_sender", 4 * 1024, NULL, 5, NULL,
                                NEXLINK_CPU_WIRELESS) != pdPASS) {
        return ESP_ERR_NO_MEM;
    }
...
{
    if (!data || length == 0 || !s_frontend.cdc_rx || !s_cdc_uart_to_usb) return ESP_ERR_INVALID_STATE;
    return xRingbufferSend(s_cdc_uart_to_usb, data, length, 0) == pdTRUE ? ESP_OK : ESP_ERR_NO_MEM;
}
